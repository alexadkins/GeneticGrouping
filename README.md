...
column plus numeric skill/preference metrics. Uses
[uv](https://docs.astral.sh/uv/) for dependencies - run scripts with
`uv run python <script>.py` from the project root. `requirements.txt` is
also included for a plain `pip install -r requirements.txt` if you'd rather
not use `uv` - it's autogenerated from `uv.lock` (`uv export --format
requirements-txt --no-hashes -o requirements.txt`), so regenerate it that
way after any dependency change rather than hand-editing it.
...
3. **Prepare the data:**
   ...
   ```
   (review `config.py` first - see "Configuring the pipeline" below)
...
expects. Set `raw_csv`/`prepared_csv` in `config.py`, then run it. If a student
...
## Configuring the pipeline (`config.py`)

Every per-semester setting for all three scripts - `prepare_data.py`,
`genetic_grouping.py`, `pair_teams.py` - lives in one file, `config.py`,
organized into sections matching which script uses what. Each script
imports from it rather than defining its own copy, so there's one place to
look regardless of which stage you're touching. (The one exception:
`hard_seats`/`allowed_seats` real names for `pair_teams.py`, which stay in
your own local, gitignored file - see "Pairing teams for shared tables"
below.)
...
- `assign_tables(teams, best_matching, score_of, ...)` - optionally assigns
  each pair to a physical table. `TABLE_PRIORITY` in `config.py` (edit per
  semester/room) is an ordered list of table labels, most-spacious
...
- **No automated test suite.** Everything in this pipeline has been
  verified through manual smoke tests during development (checking group
  sizes, avoid-constraint conflicts, real production-scale runs) rather
  than a repeatable `pytest` suite - a real regression could go unnoticed
  until someone happens to run the pipeline and eyeball the output. Worth
  building eventually, ideally alongside CI, especially since this project
  gets touched once a semester and details fade between visits.
